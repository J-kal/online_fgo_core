...
namespace fgo::factor {
  ...
  public:
    ...
    evaluateError_(const gtsam::Pose3 &poseReceiver, const gtsam::Vector &ddAmbiguity) const {
      ...
      return (gtsam::Vector1()
        << (rangeRM - rangeBM) - (rangeRI - rangeBI) + lambda_ * (ddAmbiguity(nAmbiguity_) - ddphi_)).finished();
    }
    ...
    gtsam::Vector liftValuesAsVector(const gtsam::Values &values) {
      ...
      const auto amb = values.at<gtsam::Vector>(key2());
      const auto liftedStates = (gtsam::Vector() << pose.rotation().rpy(),
        pose.translation(),
        amb).finished();
      return liftedStates;
    }
    ...
    gtsam::Values generateValuesFromStateVector(const gtsam::Vector &state) {
      ...
      try {
        ...
        values.insert(key2(), gtsam::Vector(state.block(6, 0, 1, state.rows() - 6)));
      }
