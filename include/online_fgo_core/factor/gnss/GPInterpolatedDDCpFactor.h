  ...
  public:
    ...
    evaluateError(const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1, const gtsam::Vector3 &omega1,
      ...
      if (useAutoDiff_) {
        ...
        gtsam::Point3 positionReceiver =
          poseBody.translation(&Hpose_P) + poseBody.rotation(&Hpose_Pr).rotate(lb_, &Hrho_p);
        ...
        if (lb2_.norm() == 0) {
          ...
          gtsam::Point3 positionReceiver2 =
            poseBody.translation(&Hpose_P) + poseBody.rotation(&Hpose_Pr2).rotate(lb2_, &Hrho_p2);
          rangeRM = gtsam::distance3(positionReceiver, pointMaster_, &e_RM);
          rangeRI = gtsam::distance3(positionReceiver, pointSatI_, &e_RI);
    ...
    evaluateError_(const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1, const gtsam::Vector3 &omega1,
      ...
      if (lb2_.norm() == 0) {
        ...
        gtsam::Point3 positionReceiver2 =
          poseBody.translation() + poseBody.rotation() * lb2_; //pose_Antenna wrt to Earth
        rangeRM = gtsam::distance3(positionReceiver, pointMaster_);
        rangeRI = gtsam::distance3(positionReceiver, pointSatI_);
      ...
      return (gtsam::Vector1()
        << (rangeRM - rangeBM) - (rangeRI - rangeBI) + lambda_ * (ddambiguity_j(nAmbiguity_) - ddphi_)).finished();
    }
    ...
    gtsam::Vector liftValuesAsVector(const gtsam::Values &values) {
      ...
      const auto ambI = values.at<gtsam::Vector2>(key4());
      const auto liftedStates = (gtsam::Vector() << poseI.rotation().rpy(),
        poseI.translation(),
        velI, omegaI,
        poseJ.rotation().rpy(),
        poseJ.translation(),
        velJ, omegaJ, ambI).finished();
      return liftedStates;
    }
    ...
    gtsam::Values generateValuesFromStateVector(const gtsam::Vector &state) {
      ...
      try {
        ...
        values.insert(key5(), gtsam::Pose3(gtsam::Rot3::RzRyRx(state.block<3, 1>(12, 0)),
                                           gtsam::Point3(state.block<3, 1>(15, 0))));
        values.insert(key6(), gtsam::Vector3(state.block<3, 1>(18, 0)));
        values.insert(key7(), gtsam::Vector3(state.block<3, 1>(21, 0)));
        values.insert(key4(), gtsam::Vector(state.block(24, 0, 1, state.rows() - 24)));
      }
