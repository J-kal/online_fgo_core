...
namespace fgo::factor {
  ...
  public:
    ...
    evaluateError_(const gtsam::Pose3 &pose0, const gtsam::Vector3 &vel0, const gtsam::Vector3 &omega0,
                   const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1, const gtsam::Vector3 &omega1,
                   const gtsam::Vector2 &cbd1, const gtsam::Vector &ambiguityi,
                   const gtsam::Pose3 &pose2, const gtsam::Vector3 &vel2, const gtsam::Vector3 &omega2,
      ...
      auto err = (gtsam::Vector1()
        << (range_j - range_i) + e_j * (positionReceiver_j - positionReceiver_i) + cbd1(1) * dt +
...
//std::cout << " amb " << ambiguityj(nAmbiguity_j_) - ambiguityi(nAmbiguity_i_) << std::endl;
    ...
    gtsam::Vector liftValuesAsVector(const gtsam::Values &values) {
      //ToDo: the dimension of ambiguitiy factors must be fixed!!
      const auto pose0 = values.at<gtsam::Pose3>(key1());
      const auto vel0 = values.at<gtsam::Vector3>(key2());
      const auto omega0 = values.at<gtsam::Vector3>(key3());
      const auto pose1 = values.at<gtsam::Pose3>(key4());
      const auto vel1 = values.at<gtsam::Vector3>(key5());
      const auto omega1 = values.at<gtsam::Vector3>(key6());
      const auto cbd1 = values.at<gtsam::Vector2>(key7());
      const auto amb1 = values.at<gtsam::Vector>(key8());
      const auto pose2 = values.at<gtsam::Pose3>(key9());
      const auto vel2 = values.at<gtsam::Vector3>(key10());
      const auto omega2 = values.at<gtsam::Vector3>(key11());
      const auto amb2 = values.at<gtsam::Vector>(key12());
      const auto liftedStates = (gtsam::Vector() << pose0.rotation().rpy(),
        pose0.translation(), vel0, omega0,
        pose1.rotation().rpy(),
        pose1.translation(), vel1, omega1, cbd1, amb1,
        pose2.rotation().rpy(),
        pose2.translation(), vel2, omega2, amb2).finished();
      return liftedStates;
    }
  ...
  public:
    ...
    [[nodiscard]] gtsam::Vector
    evaluateError_(const gtsam::Pose3 &pose1, const gtsam::Vector2 &cbd1, const gtsam::Vector &ambiguityi,
      ...
      auto err = (gtsam::Vector1()
        << (range_j - range_i) + e_j * (positionReceiver_j - positionReceiver_i) + cbd1(1) * dt_ +
           lambda_ * (ambiguityj(nAmbiguity_j_) - ambiguityi(nAmbiguity_i_))
           - lambda_ * (phi_j - phi_i)).finished();
