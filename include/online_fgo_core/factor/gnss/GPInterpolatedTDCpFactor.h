...
namespace fgo::factor {
  ...
  //Lyu, Bai, Lai, Wang and Huang - Optimal Time Difference-based TDCP-GPS/IMU Navigation using GraphBase Optimization
  class GPInterpolatedTDCpFactor : public gtsam::NoiseModelFactor6<gtsam::Pose3, gtsam::Vector3, gtsam::Vector3,
  ...
  private:
    ...
    typedef GPInterpolatedTDCpFactor This;
  ...
  public:
    ...
    GPInterpolatedTDCpFactor() = default;

    //Constructor for between two states
    GPInterpolatedTDCpFactor(gtsam::Key point_1, gtsam::Key vel_1, gtsam::Key omega_1,
    ...
    ~GPInterpolatedTDCpFactor() override = default;
  ...
  class GPInterpolated3TDCpFactor : public fgo::NoiseModelFactor9<gtsam::Pose3, gtsam::Vector3, gtsam::Vector3,
  ...
  private:
    ...
    std::shared_ptr<fgo::models::GPInterpolator> GPbasei_, GPbasej_; //Interpolator
    typedef GPInterpolated3TDCpFactor This;
  ...
  public:
    ...
    GPInterpolated3TDCpFactor() = default;

    GPInterpolated3TDCpFactor(gtsam::Key point_i, gtsam::Key vel_i, gtsam::Key omega_i,
      ...
      pointSatMj_(pointMaster_j), pointSatIj_(pointSatI_j), pointBase_(pointBase), GPbasei_(interpolator_i),
      GPbasej_(interpolator_j) {
      phi_ji = ddphi_j - ddphi_i;
    }

    ~GPInterpolated3TDCpFactor() override = default;
    ...
    evaluateError(const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1, const gtsam::Vector3 &omega1,
      ...
      if (H3) {
        *H3 = gtsam::numericalDerivative11<gtsam::Vector1, gtsam::Vector3>(
          std::bind(&This::evaluateError_, this, pose1, vel1, std::placeholders::_1, pose2, vel2, omega2, pose3, vel3,
                    omega3), omega1);
      }
      ...
      if (H6) {
        *H6 = gtsam::numericalDerivative11<gtsam::Vector1, gtsam::Vector3>(
          std::bind(&This::evaluateError_, this, pose1, vel1, omega1, pose2, vel2, std::placeholders::_1, pose3, vel3,
                    omega3), omega2);
      }
      ...
      if (H9) {
        *H9 = gtsam::numericalDerivative11<gtsam::Vector1, gtsam::Vector3>(
          std::bind(&This::evaluateError_, this, pose1, vel1, omega1, pose2, vel2, omega2, pose3, vel3,
                    std::placeholders::_1), omega3);
      }

      return evaluateError_(pose1, vel1, omega1, pose2, vel2, omega2, pose3, vel3, omega3);
    }

    [[nodiscard]] gtsam::Vector1
    evaluateError_(const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1, const gtsam::Vector3 &omega1,
                   const gtsam::Pose3 &pose2, const gtsam::Vector3 &vel2, const gtsam::Vector3 &omega2,
...
namespace gtsam {
  ...
  template<>
  struct traits<fgo::factor::GPInterpolatedTDCpFactor> :
    public Testable<fgo::factor::GPInterpolatedTDCpFactor> {
  };

}

#endif //ONLINE_FGO_GPINTERPOLATEDTDCPFACTOR_H
