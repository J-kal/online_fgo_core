  ...
  public:
    ...
    PVTFactor(const gtsam::Key &poseKey, const gtsam::Key &velKey, const gtsam::Key &omegaKey,
              ...
              bool useAutoDiff = false) : Base(model, poseKey, velKey, omegaKey), posMeasured_(positionMeasured),
                                          velMeasured_(velocityMeasured), velocityFrame_(velocityFrame), lb_(lb),
    ...
    {*/
      ...
      if (velocityFrame_ == MeasurementFrame::ECEF) {
        ...
        if (H2)
          *H2 = (gtsam::Matrix63() << gtsam::Matrix33::Zero(), gtsam::Matrix33::Identity()).finished();  // Shape 6 x 3
        ...
        if (H1)
          *H1 = (gtsam::Matrix66() << Hpose_P + Hpose_r * Hpose_Pr, jac.block<3, 3>(0, 0) *
                                                                    (Hdrho_t * Hpose_Pr + Hdrho_p)).finished();
        if (H2) *H2 = (gtsam::Matrix63() << gtsam::Matrix33::Zero(), jac.block<3, 3>(0, 0)).finished();  // Shape 6 x 3
        if (H3)
          *H3 = (gtsam::Matrix63() << gtsam::Matrix33::Zero(), jac.block<3, 3>(0, 0) *
                                                               Hdrho_p).finished();  // Shape 6 x 3
      } else {
