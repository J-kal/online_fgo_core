...
#include "include/utils/NavigationTools.h"
#include <cmath>
#include <fstream>
#include <iostream>
  ...
  namespace factor {
    ...
    public:
      ...
      DDPrDrFactor(gtsam::Key point_i, gtsam::Key vel_i, const double &ddPseuRa,
        ...
        pointSatI_(pointSatI), velSatI_(velSatI), pointBase_(pointBase), lb_(lb), omega_(omega),
      ...
      [[nodiscard]] gtsam::Vector evaluateError(const gtsam::Pose3 &poseBody, const gtsam::Vector3 &velBody,
        ...
        if (useAutoDiff_) {
          ...
          //calculate position
          gtsam::Vector3 positionReceiver =
            poseBody.translation() + poseBody.rotation() * lb_; //pose_Antenna wrt to Earth
          //calc vel
          ...
          if (H1 || H2) {
            ...
            if (H1)
              *H1 = (gtsam::Matrix26() << dPrdRot, ((e_RM - e_RI) - (e_BM - e_BI)) *
                                                   rotation /*THIS ROT ONLY CORRECTION*/,
