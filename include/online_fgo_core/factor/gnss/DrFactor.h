...
namespace fgo::factor {
  ...
  public:
     ...
     */
    DrFactor(gtsam::Key pose_i, gtsam::Key vel_i, gtsam::Key cbd_i, const double &measdRho,
             const gtsam::Vector3 &satXYZ,
    ...
    evaluateError(const gtsam::Pose3 &pose, const gtsam::Vector3 &vel, const gtsam::Vector2 &cbd,
      ...
      if (useAutoDiff_) {
        ...
        gtsam::Point3 P_eA_e = pose.translation() + pose.rotation().rotate(lb_,
    ...
    gtsam::Vector liftValuesAsVector(const gtsam::Values &values) {
      ...
      const auto cbd = values.at<gtsam::Vector2>(key3());
      const auto liftedStates = (gtsam::Vector(11) << pose.rotation().rpy(),
        pose.translation(),
        vel, cbd).finished();
      return liftedStates;
    }
    ...
    gtsam::Values generateValuesFromStateVector(const gtsam::Vector &state) {
      ...
      try {
        ...
        values.insert(key3(), gtsam::Vector2(state.block<2, 1>(9, 0)));
      }
