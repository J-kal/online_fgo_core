 ...
 */
  ...
  class GPInterpolatedDoublePose3BetweenFactor
    : public fgo::NoiseModelFactor12<gtsam::Pose3, gtsam::Vector3, gtsam::Vector3, gtsam::Pose3, gtsam::Vector3,
  ...
  public:
    ...
    gtsam::Vector liftValuesAsVector(const gtsam::Values &values) {
      ...
      const auto pose2i = values.at<gtsam::Pose3>(key7());
      const auto vel2i = values.at<gtsam::Vector3>(key8());
      const auto omega2i = values.at<gtsam::Vector3>(key9());
      const auto pose2j = values.at<gtsam::Pose3>(key10());
      const auto vel2j = values.at<gtsam::Vector3>(key11());
      const auto omega2j = values.at<gtsam::Vector3>(key12());
      const auto liftedStates = (gtsam::Vector(48) <<
                                                   pose1i.rotation().rpy(),
        pose1i.translation(),
        vel1i, omega1i,
        pose1j.rotation().rpy(),
    ...
    gtsam::Values generateValuesFromStateVector(const gtsam::Vector &state) {
      ...
      try {
        ...
        values.insert(key8(), gtsam::Vector3(state.block<3, 1>(30, 0)));
        values.insert(key9(), gtsam::Vector3(state.block<3, 1>(33, 0)));

        values.insert(key10(), gtsam::Pose3(gtsam::Rot3::RzRyRx(state.block<3, 1>(36, 0)),
                                            gtsam::Point3(state.block<3, 1>(39, 0))));
        values.insert(key11(), gtsam::Vector3(state.block<3, 1>(42, 0)));
        values.insert(key12(), gtsam::Vector3(state.block<3, 1>(45, 0)));
...
/// traits
  template<>
  struct traits<fgo::factor::GPInterpolatedDoublePose3BetweenFactor>
    : public gtsam::Testable<fgo::factor::GPInterpolatedDoublePose3BetweenFactor> {
  };
