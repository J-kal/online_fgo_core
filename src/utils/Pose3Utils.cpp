...
namespace fgo::utils {
  ...
  /* ************************************************************************** */
  Matrix6 curlyhat(const Vector6 &vec) {
    const Vector3 &omega = vec.head(3);
    const Vector3 &rho = vec.tail(3);
    const Matrix3 omega_skew = skewSymmetric(omega);
    const Matrix3 rho_skew = skewSymmetric(rho);
    return (Matrix6() << omega_skew, Matrix3::Zero(), rho_skew, omega_skew).finished();
  }
